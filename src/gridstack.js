...
(function(factory) {
    ...
    var Utils = {
        ...
        sort: function(nodes, dir, width) {
            width = width || Math.max.apply(null, nodes.map(function(node) { return node.x + node.width; }));
            dir = dir != -1 ? 1 : -1;
    ...
    GridStackEngine.prototype.getNodeDataByDOMEl = function(el) {
        return find(this.nodes, function(n) { return el.get(0) === n.el.get(0); });
    };
    ...
    GridStackEngine.prototype._fixCollisions = function(node) {
        ...
        var nn = node;
        var hasLocked = Boolean(find(this.nodes, function(n) { return n.locked; }));
        if (!this.float && !hasLocked) {
            nn = {x: 0, y: node.y, width: this.width, height: node.height};
        }
        while (true) {
            var collisionNode = find(this.nodes, Utils._collisionNodeCheck.bind({node: node, nn: nn}));
    ...
    GridStackEngine.prototype.isAreaEmpty = function(x, y, width, height) {
        ...
        var collisionNode = find(this.nodes, function(n) {
    ...
    GridStackEngine.prototype._packNodes = function() {
        ...
        if (this.float) {
            this.nodes.forEach(function(n, i) {
                ...
                var newY = n.y;
                while (newY >= n._origY) {
                    var collisionNode = find(this.nodes, Utils._didCollide.bind({n: n, newY: newY}));
                    if (!collisionNode) {
                        n._dirty = true;
                        n.y = newY;
                    }
            ...
            this.nodes.forEach(function(n, i) {
                ...
                while (n.y > 0) {
                    ...
                    if (i > 0) {
                        var collisionNode = find(this.nodes.slice(0, i), Utils._didCollide.bind({n: n, newY: newY}));
    ...
    GridStackEngine.prototype._prepareNode = function(node, resizing) {
        node = defaults(node || {}, {width: 1, height: 1, x: 0, y: 0});
    ...
    GridStackEngine.prototype.addNode = function(node, triggerAddEvent) {
        ...
        if (node.autoPosition) {
            ...
            for (var i = 0;; ++i) {
                ...
                if (!find(this.nodes, Utils._isAddNodeIntercepted.bind({x: x, y: y, node: node}))) {
                    node.x = x;
                    node.y = y;
                    break;
                }
        ...
        if (typeof triggerAddEvent != 'undefined' && triggerAddEvent) {
            this._addedNodes.push(clone(node));
        }
    ...
    GridStackEngine.prototype.removeNode = function(node, detachNode) {
        ...
        this._removedNodes.push(clone(node));
        node._id = null;
    ...
    GridStackEngine.prototype.canMoveNode = function(node, x, y, width, height) {
        ...
        if (hasLocked) {
            res &= !Boolean(find(clone.nodes, function(n) {
    ...
    GridStackEngine.prototype.endUpdate = function() {
        ...
        var n = find(this.nodes, function(n) { return n._updating; });
        if (n) {
            n._updating = false;
        }
    };
    ...
    var GridStack = function(el, opts) {
        ...
        this.opts = defaults(opts || {}, {
            ...
            resizable: defaults(opts.resizable || {}, {
                autoHide: !(opts.alwaysShowResizeHandle || false),
                handles: 'se'
            }),
            draggable: defaults(opts.draggable || {}, {
        ...
        if (this.opts.auto) {
            ...
            sortBy(elements, function(x) { return x.i; }).forEach(function(i) {
        ...
        this._updateHeightsOnResize = throttle(function() {
    ...
    GridStack.prototype._triggerAddEvent = function() {
        if (this.grid._addedNodes && this.grid._addedNodes.length > 0) {
            this.container.trigger('added', [this.grid._addedNodes.map(clone)]);
    ...
    GridStack.prototype._triggerRemoveEvent = function() {
        if (this.grid._removedNodes && this.grid._removedNodes.length > 0) {
            this.container.trigger('removed', [this.grid._removedNodes.map(clone)]);
    ...
    GridStack.prototype._updateStyles = function(maxHeight) {
        if (Utils.isConstructableStyleSheetSupported()) {
            this._updateStyleRule(maxHeight);
        } else {
            this._updateStylesOnDirtyElements(maxHeight);
        }
    };
    ...
    GridStack.prototype._updateStylesOnDirtyElements = function(maxHeight) {
        ...
        for (var j = 0; j < panes.length; ++j) {
            // update styles only on dirty nodes
            var gridNode = $(panes[j]).data('_gridstack_node');
            if (gridNode && gridNode._dirty) {
    ...
    GridStack.prototype._prepareElementByNode = function(el, node) {
        ...
        el
            .draggable(extend(this.opts.draggable, {
                containment: this.opts.isNested ? this.container.parent() : null,
                start: onStartMoving,
                stop: onEndMoving,
                drag: dragOrResize
            }))
            .resizable(extend(this.opts.resizable, {
                start: onStartMoving,
                stop: onEndMoving,
                resize: dragOrResize
            }));
    ...
    GridStack.prototype._init_max_height = obsolete(GridStack.prototype._initMaxHeight,
